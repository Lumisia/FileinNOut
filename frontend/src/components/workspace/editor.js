...
Redis 연결 상태 = ${status.redisAvailable === true ? '연결됨' : '연결 안 됨'}`)
  ...
  async function restoreSnapshot({ title: restoredTitle = '', blocks = [] } = {}) {
    await editor.isReady
    await blockBinding.replaceAll(blocks)
    refreshImageAssetSnapshot(blocks)
    updateTitleFromLocal(restoredTitle)
    markDirty()
  }
  ...
  async function savePost() {
    ...
    try {
      ...
      // 호출자가 실패를 인지하고 사용자에게 알릴 수 있도록 다시 던진다.
      // isDirtyRef 는 그대로 true 로 남아 변경사항이 보존된다.
      throw e
    }
  }
