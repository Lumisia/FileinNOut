...
test('Jenkins controller image includes the CI deploy toolchain', () => {
  ...
  assert.match(dockerfile, /arm64\)\s*tool_arch="arm64"/)
  assert.match(dockerfile, /dl\.k8s\.io\/release\/\$\{KUBECTL_VERSION\}\/bin\/linux\/\$\{tool_arch\}\/kubectl/)
  assert.match(dockerfile, /get\.helm\.sh\/helm-\$\{HELM_VERSION\}-linux-\$\{tool_arch\}\.tar\.gz/)
...
test('Jenkins runtime docs describe the required secure setup steps', () => {
  ...
  assert.match(readme, /host\.docker\.internal:6443/)
